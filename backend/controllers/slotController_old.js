...
const validateLocalCandidate = (name, email) => {
...
exports.bookSlot = async (req, res) => {
  try {
    ...
    if (useSharePoint && sharepointService) {
      isValidCandidate = await sharepointService.validateCandidate(name, email);
    } else {
      // Local candidate validation
      isValidCandidate = validateLocalCandidate(name, email);
    }

    if (!isValidCandidate) {
      return res.status(403).json({
        message: 'You are not authorized to book a slot. Please ensure your name and email match our candidate screening list. If you believe this is an error, please contact the recruitment team.'
      });
    }
    ...
    if (useSharePoint && sharepointService) {
      ...
      // Local JSON file booking logic (candidate already validated above)
      const slots = readSlotsFile();
    ...
    res.status(200).json({
      message: 'Slot booked successfully! Your interview has been confirmed.',
      slot: bookedSlot
    });
  } catch (error) {
