...
exports.getSlotById = async (req, res) => {
  try {
    const slot = await sharepointService.getSlotById(req.params.slotId);
    ...
    console.error('Error fetching slot from SharePoint:', error);
    res.status(500).json({
      message: 'Error fetching slot from SharePoint',
      error: error.message
    });
  }
};
...
exports.bookSlot = async (req, res) => {
  try {
    ...
    const bookedSlot = await sharepointService.bookSlot(slotId, name.trim(), email.toLowerCase());
    ...
    res.status(500).json({
      message: 'Error booking slot in SharePoint',
      error: error.message
    });
  }
};
...
exports.addSlot = async (req, res) => {
  try {
    ...
    const addedSlot = await sharepointService.addNewSlot(newSlotData);
    ...
    res.status(500).json({
      message: 'Error adding slot to SharePoint',
      error: error.message
    });
  }
};
